...
def scan_sent(address, app_password, months_back=24, keyword="invoice", limit=400):
    ...
    ids = []
    for crit in (("SINCE", since, "SUBJECT", keyword), ("SINCE", since)):
        try:
...
def scan_invoice_attachments(address, app_password, months_back=24, limit=150):
    ...
    # Gmail's own search syntax finds attachments reliably; fall back to plain IMAP.
    ids = []
    attempts = [
        ("X-GM-RAW", ("SINCE", since, "X-GM-RAW", '"has:attachment filename:pdf"')),
        ("subject",  ("SINCE", since, "SUBJECT", "invoice")),
        ("since",    ("SINCE", since,)),
    ]
    for _label, crit in attempts:
        try:
            typ, data = m.search(None, *crit)
            if typ == "OK" and data and data[0]:
                ids = data[0].split()[-limit:]
